...
class serviceprovider():
    ...
    def create_product(self, id,name, price,description,stockQuantity):
        con = mysql.connector.connect( user="root", password="venkat", database="ecommerce_application",port="3305",auth_plugin='mysql_native_password')
        product_id = id
        try: 
            query = f"INSERT INTO products (product_id ,name, price,description,stockQuantity) VALUES ('{product_id}','{name}', '{price}','{description}', '{stockQuantity}')"
            c = con.cursor()
            c.execute(query)
            con.commit()
    ...
    def addTocart(self,id, customer_id,product_id, quantity):
        ...
        try:
            ...
            checking_query = f"SELECT stockQuantity FROM products WHERE product_id = {product_id}"
            c = con.cursor()
            ...
            if int(stock_quantity) >= int(quantity):
                ...
                update_query = f"UPDATE products SET stockQuantity = stockQuantity - {quantity} WHERE product_id = {product_id}"
